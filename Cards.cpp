...
void Card::play(Deck& deck, Hand& hand) { //uses a card from a hand, returns it to deck
    ...
    while (useCard){ // loops until turn is over
        ...
        if (hand.totalCardsHand() <=0){ // case if hand is empty
        ...
        useCard = false;
            ...
            if (answer == 'Y'){
                ...
                cin >> choice;
                if (choice < 0 || choice > hand.totalCardsHand()){ // checks for valid card number
                    cout << "Invalid Card.\n\n";
                }
...
Card Deck::draw(){
    if (totalCardsDeck() <= 0){ // prevents a draw if the deck is empty
        ...
        cards[take] = cards[totalCardsDeck()-1]; // places the last card in tha array to the position of the removed card
        cards[totalCardsDeck()-1] = NULL;

        int newSize = totalCardsDeck()-1;
        updateDeck(newSize); // updates the number of cards in deck
...
void Deck::returnToDeck(Card& card) { // adds a card to the end of the deck
    cards[totalCardsDeck()] = card;
    updateDeck(totalCardsDeck() + 1);
}

void Deck::showDeck(){ // displays the deck
    if (this->totalCardsDeck() == 0){
         cout << "Mo cards in Deck.\n";
    }
    else{    
        cout << "Deck List\n";
        cout << "---------------------\n";
        for (int i=0;i<(this->totalCardsDeck());i++){
...
int Deck::totalCardsDeck(){ // returns the numbers of cards currently in the deck
    return cardsInDeck;
}

void Deck::updateDeck(int a){ // used to change the number of cards in the deck
    cardsInDeck =a;
}
...
void Hand::takeCard(Deck& deck){
    if (totalCardsHand() >= 20){
        cout << "All cards in hand.\n\n";
    }
    if (deck.totalCardsDeck() <= 0) {
    cout << "No cards available in deck.\n\n";
    }
    else{
        Card take = deck.draw(); // draws a card from deck
        hand[totalCardsHand()] = take; // adds the card to the hand
        updateHand(totalCardsHand()+1); // adjusts the size of the hand
    }
}

void Hand::showHand(){ // displays the hand
    if (this->totalCardsHand() == 0){
         cout << "No cards in Hand.\n\n";
    }
    else{    
        cout << "Current Hand\n";
        cout << "---------------------\n";
        for (int i=0;i<(this->totalCardsHand());i++){
...
int Hand::totalCardsHand(){ // returns the numbers of cards currently in a hand
    return cardsInHand;
}

void Hand::updateHand(int a){ // used to change the number of cards in a hand
    cardsInHand = a;
}

Card Hand::handToDeck(int a,Deck& deck){ // returns a card that was removed from a hand, but returned to the deck
    if ((a < 0 || a >= totalCardsHand())){
        //cout << totalCardsHand();
        ...
        hand[index] = hand[totalCardsHand()-1];// positions the last card in the hand to the position of the removed card
        hand[totalCardsHand()-1] = NULL;

        int newSize = totalCardsHand()-1;
        updateHand(newSize); // adjusts the number of cards
